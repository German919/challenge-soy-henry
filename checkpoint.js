...
function stringMasLarga(strings) {
 ...
 // let palabras = ['mucho', 'muchomucho', 'muchomuchomucho', 'muchomucho muchomuchoa'];
...
function sumArray(array, n) {
  ...
  for(let i=0; i<=array.length; i++) {
     array[0] + array[i+1] == n ?  coincidencia = true : " "
  }
  return coincidencia;
...
function pluck(array, propiedad) {
  ...
  // Tu código acá:
   let  newArray = [];  
    array.map(producto=> newArray.push( propiedad === "name" ? producto.name : producto.price));
    return newArray
...
function crearClasePersona() {
  class Persona {
    ...
    getFriends() {
      ...
      // Tu código aca:
        let nombres=[];
        this.amigos.map(amigo=> nombres.push(amigo.nombre))
        return nombres;
    ...
    getPromedioEdad() {
      ...
      let  cantidad = 0;
        ...
        this.amigos.map(amigo => (
            sumatoriaEdad += amigo.edad,
            cantidad += 1
        ))
        let promedio = sumatoriaEdad / cantidad;
